...
// Type descriptor for [Bytes<32>, ZswapCoinPublicKey] to compute transientCommit
const bytesType = new CompactTypeBytes(32);
const zkpType = {
  alignment() { return bytesType.alignment(); },
  fromValue(v: any) { return { bytes: bytesType.fromValue(v) }; },
  toValue(v: any) { return bytesType.toValue(v.bytes); }
};
const commitTupleType = {
  alignment() { return bytesType.alignment().concat(zkpType.alignment()); },
  fromValue(v: any) { return [bytesType.fromValue(v), zkpType.fromValue(v)] as [Uint8Array, { bytes: Uint8Array }]; },
  toValue(v: any) { return bytesType.toValue(v[0]).concat(zkpType.toValue(v[1])); }
};

export function computeCommitment(domain: Uint8Array, owner: { bytes: Uint8Array }, rand: bigint): bigint {
  return transientCommit(commitTupleType as any, [domain, owner], rand);
}
...
export class NSSimulator {
  ...
  public commitDomain(commitment: bigint): Ledger {
    this.circuitContext = this.contract.impureCircuits.commit_domain(
      this.circuitContext,
      commitment
    ).context;
    return ledger(this.circuitContext.currentQueryContext.state);
  ...
  public cancelCommitment(domain: Uint8Array, owner: { bytes: Uint8Array }, rand: bigint): Ledger {
    this.circuitContext = this.contract.impureCircuits.cancel_commitment(
      this.circuitContext,
      domain,
      owner,
      rand
  ...
  public buyDomainFor(owner: { bytes: Uint8Array }, domain: Uint8Array, len: bigint, resolver: { bytes: Uint8Array }, rand: bigint): Ledger {
